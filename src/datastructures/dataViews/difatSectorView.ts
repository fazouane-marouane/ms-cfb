...
export class DifatSectorView {
  constructor(sector: SectorView) {
    let byteOffset = sector.buffer.byteLength - 4
    this.partialDifatArrayView = new Uint32Array(sector.buffer, 0, byteOffset/4) // 4 * byteOffset / 4 = byteOffset
    this.nextDifatSectorIndexView = new Uint32Array(sector.buffer, byteOffset, 1) // 1 byte
  }
