...
export class CFB {
  ...
  private buildFatSectors(header: Header): void {
    const {sectors} = this
    this.fatSectors = this.getDifatArray(header)
  ...
  private getDifatArray(header: Header): number[] {
    ...
    while (currentIndex !== SectorType.ENDOFCHAIN && result.length < numberOfFatSectors) {
  ...
  private buildDirectoryEntries(header: Header): void {
    ...
    if (startOfDirectoryChain !== SectorType.ENDOFCHAIN) {
      const chain = simpleBuildChain(startOfDirectoryChain, directoryChainLength, this.fatSectors, this.sectors)
  ...
  private buildMiniFatSectors(header: Header): void {
    ...
    if (startOfMiniFat !== SectorType.ENDOFCHAIN) {
      ...
      const miniFatView = simpleBuildChain(startOfMiniFat, header.getMiniFatChainLength(), this.fatSectors, this.sectors)
