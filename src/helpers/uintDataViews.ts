  ...
  public get(littleEndianness: boolean = true): number {
    ...
    switch (this.size) {
      ...
      default:
        // 64
        const [v1, v2] = [
          dataview.getUint32(byteOffset, littleEndianness),
        ...
        return (
          (littleEndianness ? v2 : v1) * 0x100000000 +
          (littleEndianness ? v1 : v2)
        );
    }
  }
  ...
  public set(value: number, littleEndianness: boolean = true): void {
    ...
    switch (this.size) {
      ...
      default:
        ...
        dataview.setUint32(
          byteOffset + (littleEndianness ? 0 : 4),
          v1,
          littleEndianness
        );
        dataview.setUint32(
          byteOffset + (littleEndianness ? 4 : 0),
          v2,
          littleEndianness
        );
        break;
...
export class UintArrayDataView {
  ...
  // tslint:disable-next-line:no-reserved-keywords
  public get(littleEndianness: boolean = true): number[] {
    return this.dataviews.map((dataview: UintDataView) =>
      dataview.get(littleEndianness)
    );
  }
...
export function createUintDataView(
  view: DataView,
  byteOffset: number,
  size: UintSize
): UintDataView {
...
export function createUintArrayDataView(
  view: DataView,
  byteOffset: number,
  length: number,
  size: UintSize
): UintArrayDataView {
  return new UintArrayDataView(
    sliceView(view, byteOffset, (length * size) / 8),
    size
  );
}
