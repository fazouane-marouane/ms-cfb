...
import {
  bytesOrderView,
  difatChainStartView,
  directoryChainLengthView,
  directoryChainStartView,
  fatChainLengthView,
  headerClsidView,
  headerSignatureView,
  initialDifatChainView,
  majorVersionView,
  miniFatChainLengthView,
  miniFatStartView,
  miniSectorCutoffView,
  miniSectorShiftView,
  minorVersionView,
  remainingSpaceView,
  reservedView,
...
export class Header {
  ...
  private checkDifat(): boolean {
    ...
    if (
      !firstSliceOfInitialDifat.every(
        (value: number) => value <= SectorType.MAXREGSECT
      ) ||
      !secondSliceOfInitialDifat.every(
        (value: number) => value === SectorType.FREESECT
      )
    ) {
      throwError('wrong initial difat');
    }
  ...
  private version(): string {
    return `${majorVersionView(this.buffer).get()}.${minorVersionView(
      this.buffer
    ).get()}`;
  }
